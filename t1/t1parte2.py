...
list(map(lambda s: "Sr." + s, lista_de_nomes))

# Q.2 : faz a equação para cada valor da lista
list(map((lambda n: ((3*n*2)+(2/n)+1)), lista_de_numeros))

# Q.3 : retorna uma lista que contem os nomes que terminam com "a"
list(filter(lambda ult: ult[-1:] == "a", lista_de_nomes))

# Q.4 : retorna uma lista das pessoas que nasceram depois de 1970
list(filter(lambda num: num < 2017 - 1970, idades))
